...
def concat_wavs_by_subject(example_wavs, output_folder="/data/storage1t/Turntaking/new_merged_wavs_unnorm", output_txt="merged_wavs.txt"):
    # Shuffle the list
...
if __name__ == "__main__":
    # Example usage
    example_wavs = []
    with open("sync_private/filelists/all_batch123_wavlists_unnorm.txt", "r") as f:
        example_wavs = f.read().splitlines()
    # shuffle the list
    import random
    random.shuffle(example_wavs)
    concat_wavs_by_subject(example_wavs)